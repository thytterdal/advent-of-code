...
private fun secondStar(lines: List<String>) {
    val energized = maxOf(
        lines.indices.maxOf { lines.traceBeam(0L, it.toLong(), Direction.Right) },
        lines.indices.maxOf { lines.traceBeam(lines.first().lastIndex.toLong(), it.toLong(), Direction.Left) },
        lines.first().indices.maxOf { lines.traceBeam(it.toLong(), 0L, Direction.Down) },
        lines.first().indices.maxOf { lines.traceBeam(it.toLong(), lines.lastIndex.toLong(), Direction.Up) }
    )
...
private fun List<String>.traceBeam(x: Long, y: Long, initialDirection: Direction): Int {
    ...
    while (true) {
        ...
        for (dir in directionMapping[direction to this[point.y.toInt()][point.x.toInt()]] ?: listOf(direction)) {
